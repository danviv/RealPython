...
def index():
    ...
    grid = SQLFORM.grid(db.project,create=False, links=notes,
            fields=[db.project.name, db.project.employee_name,
                    ...
                    db.project.due_date, db.project.completed],deletable=False, maxtextlength=50)
    return locals()
...
def employee():
    ...
    grid = SQLFORM.grid(db.auth_user, create=False, deletable=False,
                        fields=[db.auth_user.first_name, db.auth_user.last_name,
                                db.auth_user.email],
                        editable=False,maxtextlength=50, orderby=db.auth_user.id)
    return locals()
